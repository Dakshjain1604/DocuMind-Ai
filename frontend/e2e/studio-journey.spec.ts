...
test.describe.serial("studio journey", () => {
  ...
  test("uploads and indexes a document", async () => {
    ...
    await page.getByRole("button", { name: /Process & Index Documents/ }).click();

    await expect(page.getByText(/Indexing complete|Already indexed/)).toBeVisible({
      timeout: 150_000,
    });
    await expect(page.getByRole("button", { name: /Open Console — plate/ })).toBeEnabled();
  });

  test("asks a question and gets a streamed, cited answer", async () => {
    await page.getByRole("button", { name: /Open Console — plate/ }).click();
    await expect(page.getByRole("heading", { name: "Query Console", level: 2 })).toBeVisible();
  ...
  test("following a citation opens the Knowledge Atlas", async () => {
    ...
    await expect(page.getByRole("heading", { name: "Knowledge Cartography Atlas" })).toBeVisible();
  ...
  test("deletes the document from the library", async () => {
    await page.getByRole("button", { name: "← Close Instrument" }).click();
